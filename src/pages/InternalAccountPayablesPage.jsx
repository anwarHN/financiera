...
import InternalAccountPayableFormPage from "./InternalAccountPayableFormPage";
              ...
              {paginatedItems.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="table-actions">
                    <RowActionsMenu
                      actions={[
                        ...(canUpdate ? [{ key: "edit", label: t("common.edit"), to: `/internal-obligations/${item.id}/edit` }] : []),
                        ...(canVoidTransactions || canUpdate
                          ? [{ key: "delete", label: t("transactions.deactivate"), onClick: () => handleDelete(item.id), danger: true }]
                          : [])
                      ]}
                    />
                  </td>
      ...
      {isCreateModalOpen ? (
        ...
        >
          <div className="modal-card" onClick={(event) => event.stopPropagation()}>
            <InternalAccountPayableFormPage
              embedded
              onCancel={() => {
                const next = new URLSearchParams(searchParams);
                next.delete("create");
