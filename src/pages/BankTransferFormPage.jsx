    ...
    <div className={embedded ? "" : "module-page"}>
      ...
      <form className="crud-form" onSubmit={handleSubmit}>
        <div className="form-grid-2">
          ...
          <SelectField label={t("bankTransfers.fromBankAccount")} name="fromBankFormId" value={form.fromBankFormId} onChange={handleChange} required>
          ...
          <SelectField label={t("bankTransfers.toBankAccount")} name="toBankFormId" value={form.toBankFormId} onChange={handleChange} required>
              <option value="">{`-- ${t("transactions.selectAccountPaymentForm")} --`}</option>
              {bankForms.map((item) => (
                <option key={item.id} value={item.id}>
                  {formatPaymentFormLabel(item)}
